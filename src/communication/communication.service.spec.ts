...
describe("CommunicationService never fakes success on failure", () => {
  function buildService(
    ...
    translationProvider?: TranslationProviderService,
  ) {
  ...
  it("listMessages throws a real error instead of a fake preview thread when the read fails", async () => {
    ...
    await expect(
      service.listMessages("conv-1", customer),
    ).rejects.toBeInstanceOf(InternalServerErrorException);
  });
  ...
  it("allows the owner of the assigned truck to read the shipment conversation", async () => {
    ...
    await expect(service.listMessages("conv-1", owner)).resolves.toMatchObject({
      messages: [],
    });
  });
  ...
  it("rejects a truck owner whose vehicle is not assigned to the shipment", async () => {
    ...
    await expect(service.listMessages("conv-1", owner)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
});
...
describe("CommunicationService.transcribeVoiceNote", () => {
  function buildService(
    ...
    const translationProvider = {
      ...
      status: jest
        .fn()
        .mockReturnValue({ transcriptionEnabled: false, ...statusOverride }),
    } as unknown as TranslationProviderService;
    return new CommunicationService(
      {} as ConfigService,
      {} as PrismaService,
  ...
  it("returns an English transcript while retaining the original Nigerian-language transcript", async () => {
    const transcribe = jest
      .fn()
      .mockResolvedValue({ transcript: "Ututu oma", detectedLanguage: "ig" });
    const translate = jest
      .fn()
      .mockResolvedValue({
...
describe("CommunicationService.uploadMedia never fakes success on failure", () => {
  ...
  it('throws instead of a fake "uploaded: true" echo when the MediaAsset insert fails', async () => {
    ...
    await expect(
      service.uploadMedia(
        { kind: "DOCUMENT", base64: "AAAA", mimeType: "image/png" },
        "user-1",
      ),
    ).rejects.toBeInstanceOf(InternalServerErrorException);
  });
  ...
  it("rejects a bare local device URI with no real file content instead of storing it as if it were a real URL", async () => {
    ...
    await expect(
      service.uploadMedia(
        { kind: "VOICE_NOTE", localUri: "file:///var/mobile/recording.m4a" },
        "user-1",
      ),
  ...
  it("stores browser voice notes with a real audio extension and tolerates the legacy bucket MIME allow-list", async () => {
    const originalFetch = global.fetch;
    global.fetch = jest
      .fn()
    ...
    const queryRawUnsafe = jest.fn().mockResolvedValue([
      {
        id: "media-voice",
        kind: "VOICE_NOTE",
        url: "https://example.supabase.co/voice.webm",
        storageKey: "voice-note/user-1/voice.webm",
        label: "Voice note",
        transcript: null,
        durationSeconds: 5,
        createdAt: new Date(),
