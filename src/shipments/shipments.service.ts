...
export class ShipmentsService {
  ...
  async offerAssignment(
    ...
    await this.notifications.create({
      ...
      templateKey: 'notifyTpl.driverNewOffer',
      vars: { reference: shipment.reference },
      tone: 'INFO',
  ...
  async respondToAssignment(assignmentId: string, driverId: string, action: 'ACCEPT' | 'REJECT') {
    ...
    try {
      ...
      await this.notifications.create({
        ...
        templateKey: action === 'ACCEPT' ? 'notifyTpl.customerDriverAccepted' : undefined,
        vars: { reference: updated.shipment.reference },
  ...
  async approveShipment(shipmentId: string, reviewerId: string, actorRole: UserRole) {
    ...
    await this.notifications.create({
      ...
      templateKey: 'notifyTpl.dispatcherShipmentApproved',
      vars: { reference: shipment.reference },
      tone: 'SUCCESS',
      entity: 'Shipment',
      entityId: shipmentId,
  ...
  private async notifyAssignedDriverOfEscrowRelease(shipmentId: string, amount?: number | null, currency = 'NGN') {
    try {
      ...
      await this.notifications.create({
        ...
        templateKey: 'notifyTpl.driverEscrowReleased',
        vars: { amount: this.formatMoney(amount, currency), reference: assignment.reference ?? shipmentId },
        tone: 'SUCCESS',
        entity: 'Escrow',
        entityId: shipmentId,
        actionUrl: '/driver/earnings',
